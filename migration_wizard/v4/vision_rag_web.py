...
class T:
    ...
    INPUT="#111620"; INPUT_B="#1a2030"
    ACCENT="#5b9aff"; AC2="#7db4ff"; AC3="#3d7be0"
    ACCENT_G1="#4a8af4"; ACCENT_G2="#7c5cf5"
    OK="#34d399"; OK2="#2ab385"; OK_G1="#22c997"; OK_G2="#34d399"
    WARN="#fbbf24"; WARN2="#f59e0b"; WARN_G1="#f59e0b"; WARN_G2="#fbbf24"
    ERR="#f43f5e"; ERR2="#e11d48"; ERR_G1="#ef4444"; ERR_G2="#f43f5e"
    TXT="#f0f4f8"; TXT2="#94a3b8"; MUTE="#475569"
    BORDER="#1e293b"; B2="#334155"; B3="#0f172a"
...
def win2wsl(wp):
    if len(wp)>=2 and wp[1:3]==":\\":
        d,r=wp[0].lower(),wp[2:].replace("\\","/"); m=f"/mnt/{d}{r}"
        if Path(m).exists(): return m
    try:
        r=subprocess.run(["wslpath","-u",wp],capture_output=True,text=True,timeout=5)
...
def get_prefix():
    try:
        r=subprocess.run(["docker","volume","ls","--format","{{.Name}}"],capture_output=True,text=True,timeout=10,check=True)
        for line in r.stdout.splitlines():
            if "_hf-cache" in line: return line.replace("_hf-cache","")
    except Exception: pass
...
class State:
    def __init__(s):
        ...
        s.manifest_images=[]
        s.manifest_code={}
        s.source_steps={"disc":"pending","exp":"pending","code":"pending","bun":"pending"}
        s.target_steps={"detect":"pending","backup":"pending","import":"pending","verify":"pending"}
        s.export_dir=""

def get_export_dir():
    ts=datetime.now().strftime("%Y-%m-%d")
    folder_name=f"VisionRAG_Update_{ts}"
    dp=Path(state.dok)/folder_name
...
def auto_detect_project():
    home=Path.home()
    for p in [home/"projects"/"Vision_RAG_Git",home/"projects"/"Vision_RAG",home/"Vision_RAG_Git",home/"Vision_RAG"]:
        if is_valid_project(p): return str(p)
    proj_dir=home/"projects"
    candidates=[]
    if proj_dir.exists():
        try:
            for c in proj_dir.iterdir():
                if c.is_dir() and is_valid_project(c): candidates.append(c)
        except Exception: pass
    for m in ["/mnt/c/Users/oappe/projects","/mnt/d/Users/oappe/projects","/mnt/c/Users","/mnt/d/Users","/mnt/f","/mnt/e"]:
        mp=Path(m)
        if not mp.exists(): continue
        try:
            for c in mp.iterdir():
                if c.is_dir() and ("Vision_RAG" in c.name or "vision_rag" in c.name.lower()) and is_valid_project(c): candidates.append(c)
        except Exception: pass
    for c in candidates:
        if "vision_rag" in c.name.lower(): return str(c)
    if candidates: return str(candidates[0])
    return None

def auto_detect_dok():
    found=None
    for drive in ["/mnt/f","/mnt/d","/mnt/e","/mnt/g","/mnt/h"]:
        p=Path(drive)
        if not p.exists(): continue
        found=str(p/"VisionRAG_Update")
        if (p/"VisionRAG_Update").exists(): found=str(p/"VisionRAG_Update"); break
        break
    if not found:
        try:
            for e in Path("/mnt").iterdir():
                if e.is_dir() and e.name not in ["c","wsl","wslg"]:
                    found=str(e/"VisionRAG_Update")
                    if (e/"VisionRAG_Update").exists(): found=str(e/"VisionRAG_Update")
                    break
        except Exception: pass
    return found
...
for f in docker-compose.yml Dockerfile .env pipeline_state.json labels.json; do
    if [ -f "${{SCRIPT_DIR}}/code/${{f}}" ]; then
        if [ "${{f}}" = "pipeline_state.json" ] || [ "${{f}}" = "labels.json" ]; then
            cp "${{SCRIPT_DIR}}/code/${{f}}" "${{PROJECT_DIR}}/pipelines/" 2>/dev/null || true
        else
            cp "${{SCRIPT_DIR}}/code/${{f}}" "${{PROJECT_DIR}}/"
        fi
        ok "  ${{f}} restored"
    fi
done
...
def src_disc():
    if not state.proj:
        log_emit("source","No project path set. Please select a project folder first.","ERROR")
        state.source_steps["disc"]="error"; return
    proj=Path(state.proj)
    if not proj.exists():
        log_emit("source",f"Project path does not exist: {state.proj}","ERROR")
        state.source_steps["disc"]="error"; return
    c=proj/"docker-compose.yml"; imgs=[]
    if c.exists():
        try:
        ...
        except Exception as e: log_emit("source",f"Parse error: {e}","WARN")
    if not imgs:
        imgs=["ollama/ollama:latest","qdrant/qdrant:latest","python:3.11-alpine"]
        log_emit("source","Using fallback image list","WARN")
    state.imgs=imgs; log_emit("source",f"Discovered {len(imgs)} images:","OK")
    for img in imgs: log_emit("source",f"  • {img}","INFO")
    state.source_steps["disc"]="done"

def src_exp():
    if not state.dok:
        log_emit("source","Cannot export images: no DOK path set. Set the export destination first.","ERROR")
        state.source_steps["exp"]="error"; return
    dp=get_export_dir(); idir=dp/"images"; idir.mkdir(exist_ok=True)
    log_emit("source",f"Exporting images to {idir}...","INFO")
    m=[]
    for img in state.imgs:
        safe=img.replace("/","-").replace(":","-")+".tar.gz"; out=idir/safe
        ...
        try:
            ...
            with open(out,"wb") as f:
                gz=subprocess.Popen(["gzip","-c"],stdin=proc.stdout,stdout=f,stderr=subprocess.PIPE)
                proc.stdout.close(); gz.communicate(timeout=3600)
            if gz.returncode==0 and proc.poll()==0:
                sz=out.stat().st_size; sh=sha256f(out)
                m.append({"name":img,"file":f"images/{safe}","sha256":sh,"size_bytes":sz})
                log_emit("source",f"✓ {safe} ({hsize(sz)})","OK")
            else: log_emit("source",f"Failed {img}","ERROR")
        except Exception as e: log_emit("source",f"Exception {img}: {e}","ERROR")
    state.manifest_images=m; log_emit("source",f"Exported {len(m)}/{len(state.imgs)} images.","OK")
...
def src_code():
    if not state.export_dir:
        if state.dok:
            ...
            log_emit("source","Cannot export code: no DOK path set. Set the export destination first.","ERROR")
            state.source_steps["code"]="error"; return
    dp=Path(state.export_dir); cd=dp/"code"
    if cd.exists(): shutil.rmtree(cd,ignore_errors=True)
    cd.mkdir(parents=True)
    proj=Path(state.proj); log_emit("source",f"Exporting code from {proj}...","INFO")
    skip={".git","__pycache__",".venv","venv","node_modules",".mypy_cache",".ruff_cache","vision-rag-backups","FULL_BackUp"}
    skip_ext={".pyc",".egg-info"}
    def _copy_tree(src,dst):
        if src.name in skip or src.suffix in skip_ext: return
        if src.name==".env" and src.is_file(): return
        if src.parent.name=="cache" and src.parent.parent.name=="pipelines": return
        try:
            if src.is_dir():
                dst.mkdir(exist_ok=True)
                for item in src.iterdir(): _copy_tree(item,dst/item.name)
            else:
                try: shutil.copy(src,dst)
                except OSError:
                    try:
    ...
    try:
        _copy_tree(proj,cd)
        count=sum(1 for _ in cd.rglob("*") if _.is_file()); sz=sum(_.stat().st_size for _ in cd.rglob("*") if _.is_file())
    ...
    except Exception as e: log_emit("source",f"Code export failed: {e}","ERROR"); state.source_steps["code"]="error"
...
def src_bun():
    if not state.export_dir:
        if state.dok:
            ...
            state.source_steps["bun"]="error"; return
    dp=Path(state.export_dir); log_emit("source","Bundling wizard + scripts + manifest...","INFO")
    try:
        manifest={"export_type":"vision-rag-update","export_date":datetime.utcnow().isoformat()+"Z","source_repo":str(Path(state.proj).resolve()),"images":state.manifest_images,"code":state.manifest_code}
        with open(dp/"manifest.json","w") as f: json.dump(manifest,f,indent=2)
        log_emit("source","✓ manifest.json written","OK")
        myself=Path(__file__); _safe_copy(myself,dp/"vision_rag_web.py"); log_emit("source","✓ vision_rag_web.py copied","OK")
        gd=myself.parent
        for fn in ["requirements.txt","run-wizard.sh","VisionRAG-Wizard.desktop"]:
            src=gd/fn
            if src.exists(): _safe_copy(src,dp/fn)
...
'''
        ...
        for fn in ["import-update.sh","backup-target.sh"]:
            src=parent/fn
            if src.exists(): _safe_copy(src,dp/fn)
        for fn in ["UPDATE_GUIDE.md","TARGET_MACHINE_SAFETY_RUNBOOK.md","TARGET_QUICK_CHECKLIST.md"]:
            src=parent/fn
            if src.exists(): _safe_copy(src,dp/fn)
        log_emit("source","═══════════════════════════════════════","OK")
    ...
    except Exception as e: log_emit("source",f"Bundling failed: {e}","ERROR"); state.source_steps["bun"]="error"

# ── Target actions (threaded) ─────────────────────────────────────────────
def tgt_detect():
    log_emit("target","Scanning for VisionRAG_Update...","INFO"); cands=[]
    for m in ["/mnt/f","/mnt/d","/mnt/e","/mnt/g"]:
        base=Path(m)
        if not base.exists(): continue
        # Check for dated subfolders first (e.g. VisionRAG_Update_2026-05-30)
        try:
            for d in sorted(base.iterdir(),reverse=True):
                if d.is_dir() and d.name.startswith("VisionRAG_Update_") and (d/"images").exists():
                    cands.append(d); break
        except Exception: pass
        # Fallback: check flat VisionRAG_Update
        p=base/"VisionRAG_Update"
        if p.exists() and (p/"images").exists(): cands.append(p)
    if cands:
        state.dok=str(cands[0]); log_emit("target",f"Found: {state.dok}","OK")
...
def tgt_backup():
    if not state.proj: return
    log_emit("target","╔══════════════════════════════════════════════════════════╗","WARN")
    log_emit("target","║  STARTING FULL SAFETY BACKUP                             ║","WARN")
    log_emit("target","╚══════════════════════════════════════════════════════════╝","WARN")
    try:
        proj=Path(state.proj); ts=datetime.now().strftime("%Y%m%d_%H%M%S")
        bd=Path.home()/"vision-rag-backups"/f"backup_{ts}"; bd.mkdir(parents=True,exist_ok=True)
        for sub in ["volumes","code","docker","docker-images"]:(bd/sub).mkdir(exist_ok=True)
        (bd/"restore-from-backup.sh").write_text(gen_restore()); (bd/"PROJECT_DIR.txt").write_text(str(proj))
        prefix=get_prefix(); (bd/"VOLUME_PREFIX.txt").write_text(prefix)
        log_emit("target",f"Backup dir: {bd}","INFO"); log_emit("target",f"Volume prefix: {prefix}","INFO")
        # Images
        log_emit("target","Backing up Docker images...","WARN")
        try:
            r=subprocess.run(["docker","images","--format","{{.Repository}}:{{.Tag}}"],capture_output=True,text=True,timeout=10,check=True)
            imgs=[l.strip() for l in r.stdout.strip().splitlines() if l.strip()]
            log_emit("target",f"Found {len(imgs)} images","INFO")
            for img in imgs:
                safe=img.replace("/","-").replace(":","-")+".tar.gz"; out=bd/"docker-images"/safe
                try:
                    proc=subprocess.Popen(["docker","save",img],stdout=subprocess.PIPE)
                    with open(out,"wb") as f:
                        gz=subprocess.Popen(["gzip","-c"],stdin=proc.stdout,stdout=f); proc.stdout.close(); gz.communicate(timeout=3600)
                    if gz.returncode==0: log_emit("target",f"  ✓ {safe}","OK")
                    else: log_emit("target",f"  ✗ {img} gzip error","WARN")
                except Exception as e: log_emit("target",f"  ✗ {img}: {e}","WARN")
        except Exception as e: log_emit("target",f"Could not list images: {e}","WARN")
        # Volumes
        log_emit("target","Backing up Docker volumes...","WARN")
        for suffix in ["hf-cache","qdrant_data","ollama","open-webui","pipelines","pip-cache"]:
            vol=f"{prefix}_{suffix}"
            try:
                res=subprocess.run(["docker","volume","inspect",vol],capture_output=True,timeout=5)
                if res.returncode!=0: log_emit("target",f"  Skipped {vol} (not found)","WARN"); continue
            except Exception: continue
            out=bd/"volumes"/f"{suffix}.tar.gz"; log_emit("target",f"  Backing up {vol}...","INFO")
            try:
                for tool in ["busybox","alpine"]:
                    result=subprocess.run(["docker","run","--rm","-v",f"{vol}:/source:ro","-v",f"{bd/'volumes'}:/dest",tool,"tar","czf",f"/dest/{suffix}.tar.gz","-C","/source","."],capture_output=True,text=True,timeout=1800)
                    if result.returncode==0 and out.exists(): sz=out.stat().st_size; log_emit("target",f"    ✓ {suffix}.tar.gz ({hsize(sz)})","OK"); break
                else: log_emit("target",f"    ✗ Failed to backup {vol}","ERROR")
            except Exception as e: log_emit("target",f"    ✗ {vol}: {e}","ERROR")
        # Code
        log_emit("target","Backing up project code...","WARN")
        for fn in ["docker-compose.yml","Dockerfile",".env",".env.example"]:
            src=proj/fn
            if src.exists(): shutil.copy2(src,bd/"code"); log_emit("target",f"  ✓ {fn}","OK")
        for fn in ["pipeline_state.json","labels.json"]:
            src=proj/"pipelines"/fn
            if src.exists(): shutil.copy2(src,bd/"code"); log_emit("target",f"  ✓ pipelines/{fn}","OK")
        pdf=proj/"my-pdfs"
        if pdf.exists():
            listing="\n".join(str(p) for p in pdf.iterdir()); (bd/"code"/"my-pdfs-listing.txt").write_text(listing); log_emit("target","  ✓ my-pdfs listing saved","OK")
        try:
            subprocess.run(["docker","ps","--format","table {{.Names}}\t{{.Image}}\t{{.Status}}"],stdout=open(bd/"docker"/"containers-list.txt","w"),timeout=10,check=True)
            subprocess.run(["docker","volume","ls","--format","{{.Name}}"],stdout=open(bd/"docker"/"volumes-list.txt","w"),timeout=10,check=True)
        except Exception: pass
        total=sum(f.stat().st_size for f in bd.rglob("*") if f.is_file())
        log_emit("target","═══════════════════════════════════════","OK"); log_emit("target","BACKUP COMPLETE","OK"); log_emit("target",f"Location: {bd}","WARN"); log_emit("target",f"Total Size: {hsize(total)}","OK"); log_emit("target","═══════════════════════════════════════","OK")
        state.bkp_path=str(bd); state.bkp_done=True
        state.target_steps["backup"]="done"
    except Exception as e: log_emit("target",f"Backup failed: {e}","ERROR"); state.target_steps["backup"]="error"

def tgt_import():
    if not state.bkp_done: return
    dok=Path(state.dok); tgt=Path(state.proj); prefix=get_prefix()
    idir=dok/"images"
    if not idir.exists(): log_emit("target","No images/ on DOK!","ERROR"); return
    tars=sorted(idir.glob("*.tar.gz")); log_emit("target",f"Found {len(tars)} images to load","INFO"); loaded=0
    for tb in tars:
        log_emit("target",f"Loading {tb.name}...","INFO")
        try:
            r=subprocess.run(["docker","load","-i",str(tb)],capture_output=True,text=True,timeout=3600)
            if r.returncode==0: log_emit("target",f"  ✓ {tb.name}","OK"); loaded+=1
            else: log_emit("target",f"  ✗ {tb.name}: {r.stderr}","ERROR")
        except Exception as e: log_emit("target",f"  ✗ {tb.name}: {e}","ERROR")
    log_emit("target",f"Loaded {loaded}/{len(tars)} images","OK")
    ef=tgt/".env"
    if ef.exists(): shutil.copy2(ef,tgt/f".env.backup.{datetime.now().strftime('%Y%m%d_%H%M%S')}"); log_emit("target","Backed up existing .env","OK")
    sc=dok/"code"
    if not sc.exists(): log_emit("target","No code/ on DOK!","ERROR"); return
    log_emit("target","Copying updated code...","INFO")
    try:
        rs=shutil.which("rsync")
        if rs:
            r=subprocess.run(["rsync","-a","--delete",f"{sc}/",f"{tgt}/"],capture_output=True,text=True,timeout=60)
            if r.returncode==0: log_emit("target","Code updated (rsync)","OK")
            else: log_emit("target",f"rsync warning: {r.stderr}","WARN")
        else:
            for item in sc.iterdir():
                ...
                else: shutil.copy2(item,di)
            log_emit("target","Code updated (shutil)","OK")
    except Exception as e: log_emit("target",f"Code copy failed: {e}","ERROR"); return
    ep=tgt/".env"
    if ep.exists():
        content=ep.read_text()
        if "COMPOSE_PROJECT_NAME=" in content:
            lines=content.splitlines(); nl=[]
            for line in lines:
                if line.startswith("COMPOSE_PROJECT_NAME="): nl.append(f"COMPOSE_PROJECT_NAME={prefix}")
                else: nl.append(line)
            ep.write_text("\n".join(nl)+"\n"); log_emit("target",f"Updated COMPOSE_PROJECT_NAME={prefix}","OK")
        else:
            with open(ep,"a") as f: f.write(f"\n# Docker Compose project name\nCOMPOSE_PROJECT_NAME={prefix}\n")
            log_emit("target",f"Added COMPOSE_PROJECT_NAME={prefix}","OK")
    else:
        ex=tgt/".env.example"
        if ex.exists(): shutil.copy2(ex,ep); open(ep,"a").write(f"\nCOMPOSE_PROJECT_NAME={prefix}\n"); log_emit("target","Created .env from .env.example","OK")
    log_emit("target","═══════════════════════════════════════","OK"); log_emit("target","IMPORT COMPLETE","OK"); log_emit("target","═══════════════════════════════════════","OK")
    state.target_steps["import"]="done"

def tgt_verify():
    proj=Path(state.proj); prefix=get_prefix(); results={}
    log_emit("target","╔══════════════════════════════════════════════════════════╗","INFO")
    log_emit("target","║  RUNNING AUTOMATED VERIFICATION                          ║","INFO")
    log_emit("target","╚══════════════════════════════════════════════════════════╝","INFO")
    # Containers
    log_emit("target","Checking container status...","INFO")
    try:
        r=subprocess.run(["docker","compose","ps","--format","table {{.Name}}\t{{.Image}}\t{{.Status}}"],cwd=str(proj),capture_output=True,text=True,timeout=15)
        if r.returncode==0 and "Up" in r.stdout: results["containers"]=(True,r.stdout[:400]); log_emit("target","✓ Containers running","OK")
        else: results["containers"]=(False,"No containers showing 'Up'")
    except Exception as e: results["containers"]=(False,str(e))
    # Pipeline
    log_emit("target","Checking pipeline (localhost:8082)...","INFO")
    try:
        r=subprocess.run(["curl","-s","http://localhost:8082/status"],capture_output=True,text=True,timeout=10)
        if r.returncode==0 and r.stdout.strip(): results["pipeline_status"]=(True,r.stdout[:300]); log_emit("target","✓ Pipeline responded","OK")
        else: results["pipeline_status"]=(False,"No response")
    except Exception as e: results["pipeline_status"]=(False,str(e))
    # ColQwen2
    log_emit("target","Checking for 'ready' in pipeline logs...","INFO")
    try:
        r=subprocess.run(["docker","logs","--tail","30","open-webui-pipelines"],capture_output=True,text=True,timeout=10)
        logs=r.stdout if r.returncode==0 else ""
        if "ready" in logs.lower(): results["colqwen_ready"]=(True,"Pipeline ready"); log_emit("target","✓ ColQwen2 pipeline ready","OK")
        else: results["colqwen_ready"]=(False,"Still loading (wait 2-3 min)"); log_emit("target","⚠ ColQwen2 still loading","WARN")
    except Exception as e: results["colqwen_ready"]=(False,str(e))
    # Ollama
    log_emit("target","Checking Ollama models...","INFO")
    try:
        r=subprocess.run(["curl","-s","http://localhost:11434/api/tags"],capture_output=True,text=True,timeout=10)
        if r.returncode==0:
            data=json.loads(r.stdout); models=[m.get("name","?") for m in data.get("models",[])]
            results["ollama_models"]=(True,f"{len(models)} models"); log_emit("target",f"✓ Ollama: {len(models)} models","OK")
        else: results["ollama_models"]=(False,"No response")
    except Exception as e: results["ollama_models"]=(False,str(e))
    # hf-cache
    log_emit("target","Checking hf-cache volume...","INFO")
    try:
        vol=f"{prefix}_hf-cache"
        r=subprocess.run(["docker","run","--rm","-v",f"{vol}:/vol:ro","busybox","sh","-c","find /vol -type f | wc -l"],capture_output=True,text=True,timeout=10)
        count=r.stdout.strip()
        if r.returncode==0 and count.isdigit() and int(count)>0: results["hf_cache"]=(True,f"{count} files"); log_emit("target",f"✓ hf-cache: {count} files","OK")
        else: results["hf_cache"]=(False,"Empty")
    except Exception as e: results["hf_cache"]=(False,str(e))
    # .env
    ...
    if ep.exists():
        ...
        if "COMPOSE_PROJECT_NAME=" in content:
            for line in content.splitlines():
                if line.startswith("COMPOSE_PROJECT_NAME="): val=line.split("=",1)[-1].strip(); results["env_ok"]=(True,f"COMPOSE_PROJECT_NAME={val}"); log_emit("target",f"✓ .env configured ({val})","OK"); break
        else: results["env_ok"]=(False,"Missing COMPOSE_PROJECT_NAME")
    else: results["env_ok"]=(False,".env not found")
    # Volumes
    log_emit("target","Checking critical volumes...","INFO"); missing=[]
    for suffix in ["hf-cache","qdrant_data","ollama","open-webui"]:
        vol=f"{prefix}_{suffix}"
        try:
            r=subprocess.run(["docker","volume","inspect",vol],capture_output=True,timeout=5)
            if r.returncode!=0: missing.append(vol)
        except Exception: missing.append(vol)
    if missing: results["volumes_ok"]=(False,f"Missing: {', '.join(missing)}"); log_emit("target",f"✗ Missing: {', '.join(missing)}","ERROR")
    else: results["volumes_ok"]=(True,"All present"); log_emit("target","✓ All critical volumes present","OK")
    state.verify=results; okc=sum(1 for v in results.values() if v[0])
    log_emit("target",f"Verification: {okc}/{len(results)} passed","OK" if okc==len(results) else "WARN")
    state.target_steps["verify"]="done"

def emergency_restore():
    if not state.bkp_path or not Path(state.bkp_path).exists(): return False
    ...
    try:
        ...
        for line in proc.stdout: log_emit("target",line.rstrip(),"INFO")
        proc.wait()
        return proc.returncode==0
    except Exception as e: log_emit("target",f"Restore failed: {e}","ERROR"); return False
...
function renderTarget(){
  ...
  ${stepCard(3,'Import Update from DOK','Load new images and copy updated code. Volumes preserved.','import',ts.import,'tgtImport()',ts.backup!=='done')}
...
function stepCard(n,title,desc,key,state,onclick,disabled=false,errTitle=false){
  let barCls='',badgeCls='',titleCls='',btnCls='',btnText='Run',btnDisabled=disabled;
  if(state==='done'){barCls='ok';badgeCls='ok';titleCls='ok';btnCls='done';btnText='✓ Complete';btnDisabled=true}
...
async function tgtImport(){if(!await saveTgtPaths())return;await runStep('target','import','/api/target/import')}
...
def api_auto_detect():
    ...
    if _detect_cache["project"] is None:
        ...
        _detect_cache={"project":p,"dok":_detect_cache["dok"],"proj_valid":is_valid_project(p) if p else False}
    return jsonify({"path":_detect_cache["project"],"valid":_detect_cache["proj_valid"]})
...
def api_set_project():
    ...
    _detect_cache["project"]=path; _detect_cache["proj_valid"]=valid
...
def api_browse():
    st="/mnt"
    for c in ["/mnt/f","/mnt/d","/mnt/c/Users"]:
        if Path(c).exists(): st=c; break
    try:
        r=subprocess.run(["zenity","--file-selection","--directory","--filename",st],capture_output=True,text=True,timeout=60)
        if r.returncode==0:
            p=r.stdout.strip()
...
def api_win2wsl():
    ...
    wp=d.get("path","").strip()
    c=win2wsl(wp)
    if c: return jsonify({"path":c,"valid":is_valid_project(c)})
    return jsonify({"path":None})
...
def api_src_disc():
    def run():
        src_disc()
    t=threading.Thread(target=run,daemon=True); t.start(); t.join(timeout=30)
    return jsonify({"ok":state.source_steps["disc"]=="done"})

@app.route("/api/source/export",methods=["POST"])
def api_src_exp():
    if not state.dok: state.dok=auto_detect_dok() or ""
    def run():
        src_exp()
    t=threading.Thread(target=run,daemon=True); t.start(); t.join(timeout=7200)
...
def api_src_code():
    def run():
        src_code()
    t=threading.Thread(target=run,daemon=True); t.start(); t.join(timeout=300)
...
def api_src_bun():
    def run():
        src_bun()
    t=threading.Thread(target=run,daemon=True); t.start(); t.join(timeout=60)
...
def api_tgt_detect():
    def run():
        tgt_detect()
    t=threading.Thread(target=run,daemon=True); t.start(); t.join(timeout=15)
...
def api_tgt_import():
    def run():
        tgt_import()
    t=threading.Thread(target=run,daemon=True); t.start(); t.join(timeout=7200)
    return jsonify({"ok":state.target_steps["import"]=="done"})

@app.route("/api/target/verify",methods=["POST"])
def api_tgt_verify():
    def run():
        tgt_verify()
    t=threading.Thread(target=run,daemon=True); t.start(); t.join(timeout=120)
    results_serializable={}
    for k,(ok,detail) in state.verify.items():
        results_serializable[k]=[ok,detail]
    return jsonify({"ok":state.target_steps["verify"]=="done","results":results_serializable})
...
def api_tools_status():
    ...
    except Exception as e: result["df"]=f"Error: {e}"
    try:
        r=subprocess.run(["docker","images","--format","{{.Repository}}:{{.Tag}}\t{{.Size}}"],capture_output=True,text=True,timeout=10)
        result["images"]=r.stdout if r.returncode==0 else ""
    except Exception: pass
    try:
        r=subprocess.run(["docker","ps","-a","--format","{{.Names}}\t{{.Image}}\t{{.Status}}"],capture_output=True,text=True,timeout=10)
...
def api_tools_action():
    ...
    cmds={
        ...
        "remove-none":["bash","-c","docker images --filter 'dangling=true' -q | xargs -r docker rmi"],
        "prune-all":["docker","image","prune","-a","-f"],
    }
    if action not in cmds: return jsonify({"ok":False,"error":"Unknown action"})
    try:
        r=subprocess.run(cmds[action],capture_output=True,text=True,timeout=60)
        if r.returncode==0: return jsonify({"ok":True,"msg":r.stdout.strip()[:500]})
        else: return jsonify({"ok":False,"error":r.stderr.strip()[:500]})
...
def api_list_dir():
    ...
    try:
        p=Path(path)
        if not p.exists() or not p.is_dir(): return jsonify({"path":path,"dirs":[],"error":"Not a directory"})
        dirs=[]
    ...
    except Exception as e: return jsonify({"path":path,"dirs":[],"error":str(e)})
...
def main():
    ...
    print("="*60)
    app.run(host="0.0.0.0",port=5555,debug=False,threaded=True,extra_files=None,exclude_patterns=None)
