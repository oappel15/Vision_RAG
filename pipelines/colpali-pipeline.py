...
class Pipeline:
    class Valves(BaseModel):
        ...
        def _sanitize_stale_openrouter_model(cls, data):
            ...
            if model_value is None:
                return data
            allowed = typing.get_args(cls.model_fields["OPENROUTER_MODEL"].annotation)
    ...
    def _format_index_status(self) -> str:
        ...
        if indexed:
            ...
            for fn in indexed:
                ...
                label_str = (
                    f" [labels: {', '.join(file_labels)}]" if file_labels else ""
                )
    ...
    def _format_labels_list(self) -> str:
        ...
        if user_labels:
            ...
            for lbl in sorted(user_labels.keys(), key=str.lower):
                ...
                snippet = (
                    f'label:"{lbl}"' if " " in lbl or "/" in lbl else f"label:{lbl}"
                )
                count = len(files)
                lines.append(
        ...
        with _req.post(
            ...
            try:
                for line in resp.iter_lines():
                    ...
                    if time.monotonic() - last_content_time > STREAM_IDLE_TIMEOUT_S:
                        log.warning(
            ...
            if stream_aborted:
                log.warning("VLM stream aborted by idle watchdog (model=%s)", model)
    ...
    def _pipe_stream(self, query: str, label_filters: list = None):
        ...
        try:
            ...
            hits = self._search(
                search_query, top_k=self.valves.TOP_K, label_filters=label_filters
            )
    ...
    def pipe(self, body: dict, **kwargs):
        ...
        cleared = any(
            lbl.lower() in ("all", "clear", "none", "reset") for lbl in current_labels
        )
        ...
        if cleared:
            ...
            for msg in reversed(messages[:-1]):
                ...
                if content.strip().startswith(
                    "### Task:"
                ) or content.strip().startswith("### Instructions:"):
                    continue
