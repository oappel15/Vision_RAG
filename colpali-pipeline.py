...
class Pipeline:
    ...
    def pipe(self, body: dict, **kwargs) -> str:
        try:
            ...
            # ── Compact source previews with click-to-view ───────────
            if self.valves.SHOW_SOURCE_PAGES:
                ...
                for i, hit in enumerate(hits):
                    ...
                    if img_b64:
                        ...
                        readable_b64 = self._make_readable(img_b64, max_width=800)
                        answer += (
                            f'<a href="data:image/jpeg;base64,{readable_b64}" target="_blank" '
                            f'title="Page {page} from {source} (score: {score:.2f})">'
                            f'<img src="data:image/jpeg;base64,{thumb_b64}" '
                            f'height="120" style="display:inline-block;margin:4px;'
                            f'border:1px solid #ccc;border-radius:4px;cursor:zoom-in;" />'
                            f'</a> '
                        )
            return answer
